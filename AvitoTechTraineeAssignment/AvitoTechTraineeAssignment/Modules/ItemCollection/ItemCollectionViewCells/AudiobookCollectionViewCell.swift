...
final class AudiobookCollectionViewCell: UICollectionViewCell, ReusableCollectionCell {
    ...
    func configure(with model: Audiobook) {
        nameLabel.text = model.name
        typeLabel.text = model.wrapperType
        imageView.image = model.getImageOf()
    }
    
    private func setup() {
        contentView.backgroundColor = .green.withAlphaComponent(0.7)
        setupTypeLabel()
        setupImage()
        setupNameLabel()
    }
