...
final class AlbumInfoView: UIView, ConformingView {
    ...
    func configure(with model: Album) {
        ...
        typeLabel.text = "Type: \(model.wrapperType) \(model.collectionType)"
        artistNameLabel.text = "Author: \(model.artistName)"
        descriptionLabel.text = "Description: Album type has no description in data from API, but is has primaryGenreName: \(model.primaryGenreName), and contains \(model.trackCount) tracks"
